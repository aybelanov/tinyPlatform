...
namespace Hub.Web.Framework.Mvc.Routing
{
   ...
   /// </summary>
   public class AppRedirectResultExecutor : RedirectResultExecutor
   {
      #region Fields
      ...
      public AppRedirectResultExecutor(IActionContextAccessor actionContextAccessor,
      ...
      public override Task ExecuteAsync(ActionContext context, RedirectResult result)
      {
         ...
         if (_securitySettings.AllowNonAsciiCharactersInHeaders)
         {
            ...
            var uri = new Uri(isLocalUrl ? $"{_webHelper.GetAppLocation().TrimEnd('/')}{url}" : url, UriKind.Absolute);
