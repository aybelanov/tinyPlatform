...
namespace Hub.Web.Framework.UI
{
   ...
   /// </summary>
   public partial class AppHtmlHelper : IAppHtmlHelper
   {
      #region Fields
      ...
      public AppHtmlHelper(AppSettings appSettings,
      ...
      public virtual IHtmlContent GenerateTitle(bool addDefaultTitle = true, string part = "")
      {
         ...
         if (!string.IsNullOrEmpty(specificTitle))
            if (addDefaultTitle)
               ...
               {
                  case PageTitleSeoAdjustment.PagenameAfterPlatformname:
                     {
                        result = string.Join(_seoSettings.PageTitleSeparator, _seoSettings.DefaultTitle, specificTitle);
                     }
                     break;
                  case PageTitleSeoAdjustment.PlatformnameAfterPagename:
                  default:
                     {
      ...
      public virtual IHtmlContent GenerateScripts(ResourceLocation location)
      {
         ...
         if (woConfig.EnableJavaScriptBundling && _scriptParts[location].Any(item => !item.ExcludeFromBundle))
         {
            ...
            result.AppendFormat("<script type=\"{0}\" src=\"{1}{2}?v={3}\"></script>",
                MimeTypes.TextJavascript, pathBase, bundleAsset.Route, bundleAsset.GenerateCacheKey(_actionContextAccessor.ActionContext.HttpContext, woConfig));
         }
         ...
         foreach (var item in scripts)
         {
            ...
            result.AppendFormat("<script type=\"{0}\" src=\"{1}?v={2}\"></script>",
                MimeTypes.TextJavascript, asset.Route, asset.GenerateCacheKey(_actionContextAccessor.ActionContext.HttpContext, woConfig));
      ...
      public virtual IHtmlContent GenerateCssFiles()
      {
         ...
         if (woConfig.EnableCssBundling && _cssParts.Any(item => !item.ExcludeFromBundle))
         {
            ...
            result.AppendFormat("<link rel=\"stylesheet\" type=\"{0}\" href=\"{1}{2}?v={3}\" />",
                MimeTypes.TextCss, pathBase, bundleAsset.Route, bundleAsset.GenerateCacheKey(_actionContextAccessor.ActionContext.HttpContext, woConfig));
         }
         ...
         foreach (var item in styles)
         {
            ...
            result.AppendFormat("<link rel=\"stylesheet\" type=\"{0}\" href=\"{1}?v={2}\" />",
                MimeTypes.TextCss, asset.Route, asset.GenerateCacheKey(_actionContextAccessor.ActionContext.HttpContext, woConfig));
            result.AppendLine();
         }
      ...
      public virtual string GetRouteName(bool handleDefaultRoutes = false)
      {
         ...
         if (httpContext.GetRouteValue(AppPathRouteDefaults.SeNameFieldKey) is not null &&
             httpContext.GetRouteValue(AppPathRouteDefaults.ActionFieldKey) is string actionKey)
