...
namespace Hub.Web.Framework.TagHelpers.Shared
{
   ...
   public class AppScriptTagHelper : UrlResolutionTagHelper
   {
      #region Constants
      ...
      public AppScriptTagHelper(AppSettings appSettings,
      ...
      public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
      {
         ...
         if (Location == ResourceLocation.None)
         {
            if (!string.IsNullOrEmpty(Src))
            {
               ...
               ProcessAsset(output, _appSettings.Get<WebOptimizerConfig>());
            }
         ...
         if (string.IsNullOrEmpty(Src))
            _appHtmlHelper.AddInlineScriptParts(Location, await BuildInlineScriptTagAsync(output));
         else
            _appHtmlHelper.AddScriptParts(Location, Src, DebugSrc, ExcludeFromBundle);
