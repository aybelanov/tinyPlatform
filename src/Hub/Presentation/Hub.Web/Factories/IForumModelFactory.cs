...
public partial interface IForumModelFactory
{
   ...
   /// The task result contains the active discussions model
   /// </returns>
   Task<ActiveDiscussionsModel> PrepareActiveDiscussionsModelAsync(long forumId, int page);
   ...
   /// </returns>
   Task<ForumBreadcrumbModel> PrepareForumBreadcrumbModelAsync(long? forumGroupId, long? forumId,
       long? forumTopicId);
