...
public partial class AddressModelFactory : IAddressModelFactory
{
       ...
       Address address, string overrideAttributesXml = "")
   {
      ...
      foreach (var attribute in attributes)
      {
         ...
         {
            Id = attribute.Id,
            ControlId = string.Format(HubCommonDefaults.AddressAttributeControlName, attribute.Id),
       ...
       string overrideAttributesXml = "")
   {
      ...
      if (address == null && prePopulateWithUserFields)
      {
         ...
         model.Address1 = await _genericAttributeService.GetAttributeAsync<string>(user, AppUserDefaults.StreetAddressAttribute);
         model.Address2 = await _genericAttributeService.GetAttributeAsync<string>(user, AppUserDefaults.StreetAddress2Attribute);
         ...
         model.PhoneNumber = await _genericAttributeService.GetAttributeAsync<string>(user, AppUserDefaults.PhoneAttribute);
         model.FaxNumber = await _genericAttributeService.GetAttributeAsync<string>(user, AppUserDefaults.FaxAttribute);
      }
