...
public partial class WebHelper : IWebHelper
{
   ...
   public virtual string GetThisPageUrl(bool includeQueryString, bool? useSsl = null, bool lowercaseUrl = false)
   {
      ...
      var storeLocation = GetAppLocation(useSsl ?? IsCurrentConnectionSecured());
   ...
   /// <summary>
   /// Gets a platform host location
   /// </summary>
   /// <param name="useSsl">Whether to get SSL secured URL</param>
   /// <returns>Platform host location</returns>
   public virtual string GetPlatformHost(bool useSsl)
   {
      if (!IsRequestAvailable())
         return string.Empty;

      // grpc 
      var contentHeader = _httpContextAccessor.HttpContext.Request.Headers[HeaderNames.ContentType];
      if (!StringValues.IsNullOrEmpty(contentHeader) && contentHeader.ToString().Equals("application/grpc"))
         return string.Empty;
      ...
      var platformHost = $"{(useSsl ? Uri.UriSchemeHttps : Uri.UriSchemeHttp)}{Uri.SchemeDelimiter}{hostHeader.FirstOrDefault()}";

      //ensure that host is ended with slash
      platformHost = $"{platformHost.TrimEnd('/')}/";
   ...
   /// <returns>App location</returns>
   public virtual string GetAppLocation(bool? useSsl = null)
   {
      var appLocation = string.Empty;

      //get store host
      var appHost = GetPlatformHost(useSsl ?? IsCurrentConnectionSecured());
      ...
      //if host is empty (it is possible only when HttpContext is not available), use URL of a platform entity configured in admin area
      if (string.IsNullOrEmpty(appHost))
         appLocation = Singleton<AppSettings>.Instance.Get<HostingConfig>().HubHostUrl ?? throw new Exception("Current app location cannot be loaded");
   ...
   public virtual string ModifyQueryString(string url, string key, params string[] values)
   {
      ...
      if (isLocalUrl)
      {
         ...
         uriStr = $"{GetAppLocation().TrimEnd('/')}{(url.StartsWith(pathBase) ? url.Replace(pathBase, "") : url)}";
      }
   ...
   public virtual string RemoveQueryString(string url, string key, string value = null)
   {
      ...
      var urlHelper = _urlHelperFactory.GetUrlHelper(_actionContextAccessor.ActionContext);
      var isLocalUrl = urlHelper.IsLocalUrl(url);
      var uri = new Uri(isLocalUrl ? $"{GetAppLocation().TrimEnd('/')}{url}" : url, UriKind.Absolute);
