...
namespace Hub.Core.Rss
{
   ...
   public partial class RssFeed
   {
      ...
      public RssFeed(string title, string description, Uri link, DateTimeOffset lastBuildDate)
      {
         ...
         Description = new XElement(AppRssDefaults.Description, description);
         Link = new XElement(AppRssDefaults.Link, link);
         LastBuildDate = new XElement(AppRssDefaults.LastBuildDate, lastBuildDate.ToString("r"));
      }
      ...
      public static async Task<RssFeed> LoadAsync(Stream stream)
      {
         try
         {
            ...
            var channel = document.Root?.Element(AppRssDefaults.Channel);

            if (channel == null)
               return null;

            var title = channel.Element(AppRssDefaults.Title)?.Value ?? string.Empty;
            var description = channel.Element(AppRssDefaults.Description)?.Value ?? string.Empty;
            var link = new Uri(channel.Element(AppRssDefaults.Link)?.Value ?? string.Empty);
            var lastBuildDateValue = channel.Element(AppRssDefaults.LastBuildDate)?.Value;
            ...
            foreach (var item in channel.Elements(AppRssDefaults.Item))
            {
      ...
      public string GetContent()
      {
         ...
         var root = new XElement(AppRssDefaults.RSS, new XAttribute("version", "2.0"));
         var channel = new XElement(AppRssDefaults.Channel,
