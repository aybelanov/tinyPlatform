...
namespace Hub.Services.Common
{
   ...
   public partial class AddressService : IAddressService
   {
      ...
      /// </returns>
      public virtual async Task<int> GetAddressTotalByCountryIdAsync(long countryId)
      {
         if (countryId == 0)
            return 0;
      ...
      /// The task result contains the number of addresses
      /// </returns>
      public virtual async Task<int> GetAddressTotalByStateProvinceIdAsync(long stateProvinceId)
      {
      ...
      public virtual async Task<Address> GetAddressByIdAsync(long addressId)
      {
         return await _addressRepository.GetByIdAsync(addressId,
             cache => cache.PrepareKeyForShortTermCache(AppEntityCacheDefaults<Address>.ByIdCacheKey, addressId));
      }
