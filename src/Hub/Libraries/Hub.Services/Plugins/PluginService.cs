...
namespace Hub.Services.Plugins
{
   ...
   public partial class PluginService : IPluginService
   {
      ...
      /// IoC Ctor
      /// </summary>
      public PluginService(IUserService userService,
      ...
      /// </summary>
      /// <param name="pluginType"></param>
      /// <param name="migrationProcessType"></param>
      ...
      {
         return (await GetPluginDescriptorsAsync<TPlugin>(loadMode, user, group))
      ...
      public virtual Task<string> GetPluginLogoUrlAsync(PluginDescriptor pluginDescriptor)
      {
         ...
         var logoExtension = AppPluginDefaults.SupportedLogoImageExtensions
             .FirstOrDefault(ext => _fileProvider.FileExists(_fileProvider.Combine(pluginDirectory, $"{AppPluginDefaults.LogoFileName}.{ext}")));
         if (string.IsNullOrWhiteSpace(logoExtension))
            return Task.FromResult<string>(null);

         var pathBase = _httpContextAccessor.HttpContext.Request.PathBase.Value ?? string.Empty;
         var logoPathUrl = _mediaSettings.UseAbsoluteImagePath ? _webHelper.GetAppLocation() : $"{pathBase}/";

         var logoUrl = $"{logoPathUrl}{AppPluginDefaults.PathName}/" +
             $"{_fileProvider.GetDirectoryNameOnly(pluginDirectory)}/{AppPluginDefaults.LogoFileName}.{logoExtension}";
