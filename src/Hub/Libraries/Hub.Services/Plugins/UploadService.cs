...
namespace Hub.Services.Plugins
{
   ...
   public class UploadService : IUploadService
   {
      ...
      public UploadService(IAppFileProvider fileProvider, IThemeProvider themeProvider)
      {
      ...
      protected virtual IList<UploadedItem> GetUploadedItems(string archivePath)
      {
         ...
         var uploadedItemsFileEntry = archive.Entries
             .FirstOrDefault(entry => entry.Name.Equals(AppPluginDefaults.UploadedItemsFileName, StringComparison.InvariantCultureIgnoreCase)
      ...
      protected virtual IDescriptor UploadSingleItem(string archivePath)
      {
         ...
         var pluginsDirectory = _fileProvider.MapPath(AppPluginDefaults.Path);

         //get path to the themes directory
         var themesDirectory = string.Empty;
         if (!string.IsNullOrEmpty(AppPluginDefaults.ThemesPath))
            themesDirectory = _fileProvider.MapPath(AppPluginDefaults.ThemesPath);
         ...
         using (var archive = ZipFile.OpenRead(archivePath))
         {
            ...
            if (rootDirectories.Count != 1)
            {
               throw new Exception("The archive should contain only one root plugin or theme directory. " +
                   "For example, DefaultTheme. " +
                   $"To upload multiple items, the archive should have the '{AppPluginDefaults.UploadedItemsFileName}' file in the root");
            }
            ...
            foreach (var entry in archive.Entries)
            {
               ...
               var isPluginDescriptor = entry.FullName
                   .Equals($"{uploadedItemDirectoryName}/{AppPluginDefaults.DescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);

               //or whether it's a theme descriptor
               var isThemeDescriptor = entry.FullName
                   .Equals($"{uploadedItemDirectoryName}/{AppPluginDefaults.ThemeDescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);
               ...
               if (isPluginDescriptor)
               {
                  ...
                  if (!((PluginDescriptor)descriptor).SupportedVersions.Contains(AppVersion.CURRENT_VERSION))
                     throw new Exception($"This plugin doesn't support the current version - {AppVersion.CURRENT_VERSION}");
               }
      ...
      protected virtual IList<IDescriptor> UploadMultipleItems(string archivePath, IList<UploadedItem> uploadedItems)
      {
         ...
         using (var archive = ZipFile.OpenRead(archivePath))
         {
            foreach (var item in uploadedItems)
            {
               ...
               //ensure that the current version of tinyPlatform is supported
               if (!item.SupportedVersions?.Contains(AppVersion.CURRENT_VERSION) ?? true)
                  continue;
               ...
               if (item.Type == UploadedItemType.Plugin)
                  descriptorPath = $"{itemPath}{AppPluginDefaults.DescriptionFileName}";

               if (item.Type == UploadedItemType.Theme && !string.IsNullOrEmpty(AppPluginDefaults.ThemeDescriptionFileName))
                  descriptorPath = $"{itemPath}{AppPluginDefaults.ThemeDescriptionFileName}";
               ...
               //ensure that the plugin current version is supported
               if (descriptor is PluginDescriptor pluginDescriptor && !pluginDescriptor.SupportedVersions.Contains(AppVersion.CURRENT_VERSION))
                  continue;
      ...
      public virtual IList<IDescriptor> UploadPluginsAndThemes(IFormFile archivefile)
      {
         ...
         try
         {
            ...
            var tempDirectory = _fileProvider.MapPath(AppPluginDefaults.UploadsTempPath);
            ...
            //you can find a sample of such descriptive file in Libraries\App.Core\Plugins\Samples\
      ...
      public virtual void UploadIconsArchive(IFormFile archivefile)
      {
         ...
         try
         {
            ...
            //check if there is a folder for favicon and app icons for the platform
            var platformIconsPath = _fileProvider.GetAbsolutePath(string.Format(HubCommonDefaults.FaviconAndAppIconsPath, 0));

            CreateDirectory(platformIconsPath);

            zipFilePath = _fileProvider.Combine(platformIconsPath, archivefile.FileName);
            using (var fileStream = new FileStream(zipFilePath, FileMode.Create))
               archivefile.CopyTo(fileStream);

            ZipFile.ExtractToDirectory(zipFilePath, platformIconsPath);
         }
         finally
         {
      ...
      public virtual void UploadFavicon(IFormFile favicon)
      {
         ...
         //check if there is a folder for favicon (favicon folder is in wwwroot/icons and is called icons_{number})
         var platformFaviconPath = _fileProvider.GetAbsolutePath(string.Format(HubCommonDefaults.FaviconAndAppIconsPath, 0));

         CreateDirectory(platformFaviconPath);

         var faviconPath = _fileProvider.Combine(platformFaviconPath, favicon.FileName);
      ...
      public virtual void UploadLocalePattern(CultureInfo cultureInfo = null)
      {
         ...
         try
         {
            ...
            var ziplocalePatternPath = getPath(HubCommonDefaults.LocalePatternPath, HubCommonDefaults.LocalePatternArchiveName);
            if (!_fileProvider.GetFileExtension(ziplocalePatternPath)?.Equals(".zip", StringComparison.InvariantCultureIgnoreCase) ?? true)
               throw new Exception($"Archive '{HubCommonDefaults.LocalePatternArchiveName}' to retrieve localization patterns not found.");
            ...
            if (!(checkDirectoryExists(HubCommonDefaults.LocalePatternPath, currentCulture.Name) ||
                checkDirectoryExists(HubCommonDefaults.LocalePatternPath, currentCulture.TwoLetterISOLanguageName)))
            {
               var cultureToUse = string.Empty;

               //3. Unpack the archive into a temporary folder
               ZipFile.ExtractToDirectory(ziplocalePatternPath, getPath(HubCommonDefaults.LocalePatternPath, tempFolder));

               //4. Search in the temp unpacked archive a folder with locales by culture
               var sourcelocalePath = _fileProvider.Combine(getPath(HubCommonDefaults.LocalePatternPath, tempFolder), currentCulture.Name);
               if (_fileProvider.DirectoryExists(sourcelocalePath))
               {
                  cultureToUse = currentCulture.Name;
               }

               var sourcelocaleISOPath = _fileProvider.Combine(getPath(HubCommonDefaults.LocalePatternPath, tempFolder), currentCulture.TwoLetterISOLanguageName);
               ...
               {
                  var destlocalePath = getPath(HubCommonDefaults.LocalePatternPath, cultureToUse);
         ...
         finally
         {
            ...
            if (checkDirectoryExists(HubCommonDefaults.LocalePatternPath, tempFolder))
               _fileProvider.DeleteDirectory(getPath(HubCommonDefaults.LocalePatternPath, tempFolder));
         }
      }
      ...
      public class UploadedItem
      {
         ...
         /// <summary>
         /// Gets or sets supported versions of tinyPlatform
         /// </summary>
