...
#pragma warning restore CS1591
   ...
   protected virtual async Task<IList<LocaleStringResource>> GetAllResourcesAsync(long languageId)
   {
      ...
      {
         return from l in query.AsNoTracking()
   ...
   /// <summary>
   /// Loads resorces from XML stream
   /// </summary>
   ...
   /// <summary>
   /// Writes locale resource values to dictionary
   /// </summary>
   ...
   protected virtual async Task<IDictionary<string, string>> UpdateLocaleResourceAsync(IDictionary<string, string> resources, long? languageId = null, bool clearCache = true)
   {
      ...
      //clear cache
      if (clearCache)
         await _staticCacheManager.RemoveByPrefixAsync(AppEntityCacheDefaults<LocaleStringResource>.Prefix);
   ...
   {
      var query = from lsr in _lsrRepository.Table.AsNoTracking()
   ...
   /// </summary>
   /// <param name="languageId">Language identifier</param>
   /// <param name="loadPublicLocales">A value indicating whether to load data for the public platform only (if "false", then for admin area only. If null, then load all locales. We use it for performance optimization of the site startup</param>
   /// <returns>
   /// A task that represents the asynchronous operation
   /// The task result contains the locale string resources
   /// </returns>
   ...
   {
      var key = _staticCacheManager.PrepareKeyForDefaultCache(AppLocalizationDefaults.LocaleStringResourcesAllCacheKey, languageId);

      //get all locale string resources by language identifier
      var allLocales = await _staticCacheManager.GetAsync(key, () => (Dictionary<string, KeyValuePair<long, string>>)null);
      ...
      if (!loadPublicLocales.HasValue || allLocales != null)
      {
         var rez = allLocales ?? await _staticCacheManager.GetAsync(key, () =>
         {
            ...
            var query = from l in _lsrRepository.Table.AsNoTracking()
                        orderby l.ResourceName
                        where l.LanguageId == languageId
                        select l;
         ...
         await _staticCacheManager.RemoveAsync(AppLocalizationDefaults.LocaleStringResourcesAllPublicCacheKey, languageId);
         await _staticCacheManager.RemoveAsync(AppLocalizationDefaults.LocaleStringResourcesAllAdminCacheKey, languageId);
      ...
      key = _staticCacheManager.PrepareKeyForDefaultCache(loadPublicLocales.Value
          ? AppLocalizationDefaults.LocaleStringResourcesAllPublicCacheKey
          : AppLocalizationDefaults.LocaleStringResourcesAllAdminCacheKey,
          languageId);
      ...
      return await _staticCacheManager.GetAsync(key, () =>
      {
         ...
         query = loadPublicLocales.Value
               ? query.Where(r => !r.ResourceName.StartsWith(AppLocalizationDefaults.AdminLocaleStringResourcesPrefix))
               : query.Where(r => r.ResourceName.StartsWith(AppLocalizationDefaults.AdminLocaleStringResourcesPrefix));
       ...
       bool logIfNotFound = true, string defaultValue = "", bool returnEmptyIfNotFound = false)
   {
      ...
      if (_localizationSettings.LoadAllLocaleRecordsOnStartup)
      {
         ...
         var resources = await GetAllResourceValuesAsync(languageId, !resourceKey.StartsWith(AppLocalizationDefaults.AdminLocaleStringResourcesPrefix, StringComparison.InvariantCultureIgnoreCase));
      ...
      {
         //gradual loading
         var key = _staticCacheManager.PrepareKeyForDefaultCache(AppLocalizationDefaults.LocaleStringResourcesByNameCacheKey
         ...
         var lsr = await _staticCacheManager.GetAsync(key, () => query.FirstOrDefault());
   ...
   public virtual async Task<string> ExportResourcesToXmlAsync(Language language)
   {
      ...
      await xmlWriter.WriteAttributeStringAsync("SupportedVersion", AppVersion.CURRENT_VERSION);
   ...
   public virtual async Task ImportResourcesFromXmlAsync(Language language, StreamReader xmlStreamReader, bool updateExistingResources = true)
   {
      ...
      var localeStringResources = _lsrRepository.Table.Where(lsr => lsr.LanguageId == language.Id)
          .OrderBy(lsr => lsr.Id).ToList();

      foreach (var localeStringResource in localeStringResources)
      ...
      await _lsrRepository.BulkInsertAsync(lrsToInsertList.Values.ToList(), false);
   ...
   public virtual async Task<string> GetLocalizedSettingAsync<TSettings>(TSettings settings, Expression<Func<TSettings, string>> keySelector,
       long languageId, bool returnDefaultValue = true, bool ensureTwoPublishedLanguages = true)
   ...
   public virtual async Task<string> GetLocalizedEnumAsync<TEnum>(TEnum enumValue, long? languageId = null) where TEnum : struct
   {
      ...
      //localized value
      var workingLanguage = await _workContext.GetWorkingLanguageAsync();
      var resourceName = $"{AppLocalizationDefaults.EnumLocaleStringResourcesPrefix}{typeof(TEnum)}.{enumValue}";
   ...
   public virtual async Task<string> GetLocalizedPermissionNameAsync(PermissionRecord permissionRecord, long? languageId = null)
   {
      ...
      //localized value
      var workingLanguage = await _workContext.GetWorkingLanguageAsync();
      var resourceName = $"{AppLocalizationDefaults.PermissionLocaleStringResourcesPrefix}{permissionRecord.SystemName}";
      var result = await GetResourceAsync(resourceName, languageId ?? workingLanguage.Id, false, string.Empty, true);
   ...
   public virtual async Task AddOrUpdateLocaleResourceAsync(IDictionary<string, string> resources, long? languageId = null)
   {
      ...
      if (resourcesToInsert.Any())
      {
         ...
         await _lsrRepository.BulkInsertAsync(locales, false);
      }
       ...
       where TPlugin : IPlugin
   {
      ...
      //localized value
      var resourceName = $"{AppLocalizationDefaults.PluginNameLocaleStringResourcesPrefix}{systemName}";
