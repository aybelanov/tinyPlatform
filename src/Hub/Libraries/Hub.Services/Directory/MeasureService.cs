...
public partial class MeasureService : IMeasureService
{
       ...
       MeasureDimension targetMeasureDimension)
   {
      ...
      if (exchangeRatio == decimal.Zero)
         throw new AppException($"Exchange ratio not set for dimension [{targetMeasureDimension.Name}]");
       ...
       MeasureDimension sourceMeasureDimension)
   {
      ...
      if (exchangeRatio == decimal.Zero)
         throw new AppException($"Exchange ratio not set for dimension [{sourceMeasureDimension.Name}]");
      result /= exchangeRatio;
       ...
       MeasureWeight targetMeasureWeight)
   {
      ...
      if (exchangeRatio == decimal.Zero)
         throw new AppException($"Exchange ratio not set for weight [{targetMeasureWeight.Name}]");
      result *= exchangeRatio;
   ...
   public virtual async Task<decimal> ConvertToPrimaryMeasureWeightAsync(decimal value, MeasureWeight sourceMeasureWeight)
   {
      ...
      if (exchangeRatio == decimal.Zero)
         throw new AppException($"Exchange ratio not set for weight [{sourceMeasureWeight.Name}]");
      result /= exchangeRatio;
