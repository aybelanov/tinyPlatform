...
public class RoxyFilemanFileProvider : PhysicalFileProvider, IRoxyFilemanFileProvider
{
   ...
   private IAppFileProvider _appFileProvider;
   ...
   /// IoC Ctor
   /// </summary>
   public RoxyFilemanFileProvider(IAppFileProvider appFileProvider, IWebHostEnvironment environment)
    : base(appFileProvider.Combine(environment.WebRootPath, AppRoxyFilemanDefaults.DefaultRootDirectory))
   {
   ...
   /// IoC Ctor
   /// </summary>
   public RoxyFilemanFileProvider(IAppFileProvider defaultFileProvider, MediaSettings mediaSettings, IWebHostEnvironment environment)
    : this(defaultFileProvider, environment)
   {
   ...
   /// </summary>
   /// <param name="pathBase">The base path for the platform</param>
   ...
   public virtual async Task<RoxyFilemanConfig> GetOrCreateConfigurationAsync(string pathBase, string lang)
   {
      ...
      var filePath = _appFileProvider.GetAbsolutePath(AppRoxyFilemanDefaults.ConfigurationFile);

      //create file if not exists
      _appFileProvider.CreateFile(filePath);

      //try to read existing configuration
      var existingText = await _appFileProvider.ReadAllTextAsync(filePath, Encoding.UTF8);
      ...
      {
         FILES_ROOT = existingConfiguration?.FILES_ROOT ?? AppRoxyFilemanDefaults.DefaultRootDirectory,
         ...
         FORBIDDEN_UPLOADS = existingConfiguration?.FORBIDDEN_UPLOADS ?? string.Join(" ", AppRoxyFilemanDefaults.ForbiddenUploadExtensions),
