...
public partial class PictureService : IPictureService
{
   ...
   protected virtual Task<string> GetThumbLocalPathAsync(string thumbFileName)
   {
      ...
      if (_mediaSettings.MultipleThumbDirectories)
      {
         ...
         if (fileNameWithoutExtension != null && fileNameWithoutExtension.Length > AppMediaDefaults.MultipleThumbDirectoriesLength)
         {
            var subDirectoryName = fileNameWithoutExtension[0..AppMediaDefaults.MultipleThumbDirectoriesLength];
            thumbsDirectoryPath = _fileProvider.GetAbsolutePath(AppMediaDefaults.ImageThumbsPath, subDirectoryName);
   ...
   /// </returns>
   protected virtual Task<string> GetImagesPathUrlAsync(string platformLocation = null)
   {
      var pathBase = _httpContextAccessor.HttpContext.Request?.PathBase.Value ?? string.Empty;
      var imagesPathUrl = _mediaSettings.UseAbsoluteImagePath ? platformLocation : $"{pathBase}/";
      imagesPathUrl = string.IsNullOrEmpty(imagesPathUrl) ? _webHelper.GetAppLocation() : imagesPathUrl;
   ...
   /// The task result contains the local picture thumb path
   /// </returns>
   protected virtual async Task<string> GetThumbUrlAsync(string thumbFileName, string platformLocation = null)
   {
      var url = await GetImagesPathUrlAsync(platformLocation) + "thumbs/";
       ...
       string platformLocation = null)
   {
      ...
      {
         PictureType.Avatar => await _settingService.GetSettingByKeyAsync("Media.User.DefaultAvatarImageName", AppMediaDefaults.DefaultAvatarFileName),
         _ => await _settingService.GetSettingByKeyAsync("Media.DefaultImageName", AppMediaDefaults.DefaultImageFileName),
      };
      ...
      if (targetSize == 0)
         return await GetImagesPathUrlAsync(platformLocation) + defaultImageFileName;
      ...
      return await GetThumbUrlAsync(thumbFileName, platformLocation);
   }
   ...
   /// <param name="serviceLocation">Platform location URL; null to use determine the current platform location automatically</param>
   /// <param name="defaultPictureType">Default picture type</param>
   /// <returns>
   /// A task that represents the asynchronous operation
   /// The task result contains the picture URL
       ...
       PictureType defaultPictureType = PictureType.Entity)
   {
      ...
      return (await GetPictureUrlAsync(picture, targetSize, showDefaultPicture, serviceLocation, defaultPictureType)).Url;
   }
   ...
   {
      if (picture == null)
         return showDefaultPicture ? (await GetDefaultPictureUrlAsync(targetSize, defaultPictureType, platformLocation), null) : (string.Empty, null);
      ...
      if (picture.IsNew)
      {
         ...
         if ((pictureBinary?.Length ?? 0) == 0)
            return showDefaultPicture ? (await GetDefaultPictureUrlAsync(targetSize, defaultPictureType, platformLocation), picture) : (string.Empty, picture);
      ...
      if (targetSize == 0)
      {
         ...
         var thumbFilePath = await GetThumbLocalPathAsync(thumbFileName);
         if (await GeneratedThumbExistsAsync(thumbFilePath, thumbFileName))
            return (await GetThumbUrlAsync(thumbFileName, platformLocation), picture);
